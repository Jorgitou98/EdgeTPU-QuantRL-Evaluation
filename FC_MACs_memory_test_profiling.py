...
csv_results = open(f"FC_MACs/results/minN{minN}-maxN{maxN}-stepN{stepN}-L{L}-num_seg{num_segments}-batch{batch}-profiling.csv", "w")
...
for hidden_neurons in range(minN, maxN+1, stepN):
    csv_results = open(f"FC_MACs/results/minN{minN}-maxN{maxN}-stepN{stepN}-L{L}-num_seg{num_segments}-batch{batch}-profiling.csv", "a")
    writer_results = csv.writer(csv_results, delimiter=',')
    ...
    print(model.summary())
    # Profiling
    possible_splits = list(itertools.combinations(range(1,L+1), num_segments-1))
    #best = possible_splits[0]
    inf_times = []
    for split in possible_splits:
      split = [0] + list(split) + [L+1]
      print("split", split)
      model_segments = []
      for num_segment in range(num_segments):
        model_segments.append(Model(model.get_layer(f"dense_layer{split[num_segment]+1}").input, model.get_layer(f"dense_layer{split[num_segment+1]}").output))
      #input("continuar")
      for num_segment in range(num_segments):
      ...
      inf_time = rollout_pipeline_batch_FC.execute(model_prefix=f"FC_MACs/N{hidden_neurons}-nMACs{num_MACs}", num_segments = num_segments, steps = 1, batch = 50)
    ...
    model_segments = []
    for num_segment in range(num_segments):
      model_segments.append(Model(model.get_layer(f"dense_layer{best_split[num_segment]+1}").input, model.get_layer(f"dense_layer{best_split[num_segment+1]}").output))
    #input("continuar")
