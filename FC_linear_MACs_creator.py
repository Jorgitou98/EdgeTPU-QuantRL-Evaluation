...
csv_results = open(f"FC_linear_MACs/results/minN{minN}-maxN{maxN}-stepN{stepN}L{L}.csv", "w")
...
for hidden_neurons in range(minN, maxN+1, stepN):
    ...
    f = open(f"FC_linear_MACs/compile_info/N{hidden_neurons}-nMACs{num_MACs}_compiler", 'w')
