...
csv_results = open(f"{dir_checkpoints}/results/quant_error_{num_episodes}ep.csv", "w")
writer_results = csv.writer(csv_results, delimiter=',')
writer_results.writerow(["Num iters", "Mean rel error", "Diff actions"])
csv_results.close()

csv_results = open(f"{dir_checkpoints}/results/quant_error_{num_episodes}ep.csv", "a", buffering=1)
writer_results = csv.writer(csv_results, delimiter=',')
...
for checkpoint in checkpoints:
  ...
  mean_eror_quant, diff_act = rollout_quant_vs_dequant_steps.main(model_quant = model_quant, model_dequant = model_dequant, env_name="Pong-v0", episodes = num_episodes)
