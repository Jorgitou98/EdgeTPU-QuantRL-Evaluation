...
for checkpoint in checkpoints:
  ...
  save_model_comm = f"python model_saver.py {dir_checkpoints}/{checkpoint}/checkpoint-{iters_num} {dir_checkpoints}/exported_models/checkpoint-{iters_num}"
  subprocess.Popen(save_model_comm.split()).communicate()
  conv_tflit_comm = f"python tflite_converter_pb.py {dir_checkpoints}/exported_models/checkpoint-{iters_num} {dir_checkpoints}/exported_models/checkpoint-{iters_num}.tflite"
  subprocess.Popen(conv_tflit_comm.split()).communicate()
  quant_comm = f"python quantizer.py datasets/dataset-84.npy {dir_checkpoints}/exported_models/checkpoint-{iters_num} {dir_checkpoints}/exported_models/checkpoint-{iters_num}_quant.tflite"
