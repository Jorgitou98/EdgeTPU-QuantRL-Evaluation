...
with open(f'totalParams/totalParams[{args.mindepth}-{args.maxdepth}]-[{args.minneurons}-{args.maxneurons}].csv', 'w', newline='') as outcsv:
...
for depth in range(args.mindepth, args.maxdepth+1, args.depthstep):
  for neurons in range(args.minneurons, args.maxneurons+1, args.neuronsstep):
    ...
    train = 'python train_taxi_parametric.py --depth={} --neurons={} --gpu=gpu0 --workers=1 --save-name=model{}-{} --iters={} --total-params-file=totalParams/totalParams[{}-{}]-[{}-{}].csv'.format(depth, neurons, depth, neurons, args.iters, args.mindepth, args.maxdepth, args.minneurons, args.maxneurons)
    process = subprocess.Popen(train.split())
    output, error = process.communicate()

    export = 'python model_saver_taxi.py checkpoints/ppo/model{}-{}/checkpoint_{}/checkpoint-{} exported_models/depth-neurons/model{}-{}/checkpoint-{}-model{}-{}'.format(depth, neurons, '0'*(6-len(str(args.iters)))+str(args.iters), args.iters, depth, neurons, args.iters, depth, neurons)
    process = subprocess.Popen(export.split())
    output, error = process.communicate()

    convert = 'python tflite_converter_taxi.py exported_models/depth-neurons/model{}-{}/checkpoint-{}-model{}-{} exported_models/depth-neurons/model{}-{}/checkpoint-{}-model{}-{}.tflite'.format(depth, neurons, args.iters, depth, neurons, depth, neurons, args.iters, depth, neurons)
    ...
    quant8b = 'python quantizer8b_taxi.py datasets/dataset-model{}-{}.npy exported_models/depth-neurons/model{}-{}/checkpoint-{}-model{}-{} exported_models/depth-neurons/model{}-{}/checkpoint-{}-model{}-{}-quant8.tflite'.format(depth, neurons, depth, neurons, args.iters, depth, neurons, depth, neurons, args.iters, depth, neurons)
