...
def get_config_depth_neurons(depth, neurons):
    # La última capa está determinado cuál tiene que ser
    neurons_per_layer = int(neurons/depth)
    neurons_remainder = neurons % depth
    config = ppo.DEFAULT_CONFIG.copy()
    config['model']['fcnet_hiddens'] = []
    for i in range(depth):
      if i < neurons_remainder:
        config['model']['fcnet_hiddens'].append(neurons_per_layer+1)
      else:
        config['model']['fcnet_hiddens'].append(neurons_per_layer)
    return config

def get_config_depth_params(depth, params):
...
def main():
    ...
    parser.add_argument(
        '-n', '--neurons', required=False, type=int, default = None, help='Total neurons of the model.')
    parser.add_argument(
        '-p', '--params', required=False, type=int, default = None,  help='Number of params of the model.')
    parser.add_argument(
    ...
    if args.neurons is not None:
      config = get_config_depth_neurons(args.depth, args.neurons)
    elif args.params is not None:
      config = get_config_depth_params(args.depth, args.params)
    else:
      print("Error. Number of neurons or number of params must be given.")
      exit(1)
    ...
    # Write number of total parameter in external file
