...
for depth in [2** pow for pow in range(args.mindepthpower, args.maxdepthpower+1, args.depthsteppower)]:
  for params in [2 ** pow for pow in range(args.minparamspower, args.maxparamspower+1, args.paramsteppower)]:
    ...
    export = 'python ../savers/model_saver_taxi.py ../checkpoints/ppo/model{}-{}/checkpoint_{}/checkpoint-{} ../exported_models/depth-params/model{}-{}/checkpoint-{}-model{}-{}'.format(depth, params, '0'*(6-len(str(args.iters)))+str(args.iters), args.iters, depth, params, args.iters, depth, params)
    process = subprocess.Popen(export.split())
    output, error = process.communicate()

    convert = 'python ../converters/tflite_converter_taxi.py ../exported_models/depth-params/model{}-{}/checkpoint-{}-model{}-{} ../exported_models/depth-params/model{}-{}/checkpoint-{}-model{}-{}.tflite'.format(depth, params, args.iters, depth, params, depth, params, args.iters, depth, params)
    process = subprocess.Popen(convert.split())
    output, error = process.communicate()

    if not os.path.exists('../datasets/dataset-model{}-{}.npy'.format(depth, params)):
      create_dataset = 'python ../datasets/dataset_creator.py {} ../datasets/dataset-model{}-{}'.format(args.dim, depth, params)
      process = subprocess.Popen(create_dataset.split())
      output, error = process.communicate()

    quant8b = 'python ../quantizers/quantizer8b_taxi.py ../datasets/dataset-model{}-{}.npy ../exported_models/depth-params/model{}-{}/checkpoint-{}-model{}-{} ../exported_models/depth-params/model{}-{}/checkpoint-{}-model{}-{}-quant8.tflite'.format(depth, params, depth, params, args.iters, depth, params, depth, params, args.iters, depth, params)
    ...
    clear_log = 'rm checkpoint-{}-model{}-{}-quant8_edgetpu.log'.format(args.iters, depth, params)
