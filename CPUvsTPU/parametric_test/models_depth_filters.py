...
for depth in range(args.mindepth, args.maxdepth+1, args.depthstep):
  ...
  for filters in range(args.minfilters, args.maxfilters+1, args.filtersstep):
    ...
    rm_model_variables = f'rm -r ../exported_models/depth-filters/model{depth}-{filters}/checkpoint-{args.iters}-model{depth}-{filters}'
    process = subprocess.Popen(rm_model_variables.split())
    process.communicate()

    rm_h5 = f'rm ../exported_models/depth-filters/model{depth}-{filters}/checkpoint-{args.iters}-model{depth}-{filters}.h5'
    process = subprocess.Popen(rm_h5.split())
    process.communicate()

    rm_tfilte_quant = f'rm ../exported_models/depth-filters/model{depth}-{filters}/checkpoint-{args.iters}-model{depth}-{filters}-quant8.tflite'
    process = subprocess.Popen(rm_tfilte_quant.split())
    process.communicate()
