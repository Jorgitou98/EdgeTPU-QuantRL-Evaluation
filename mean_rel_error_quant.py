...
csv_results = open(f"{dir_checkpoints}/results/results_mean_rel_error_quant_10000steps.csv", "w")
writer_results = csv.writer(csv_results, delimiter=',')
writer_results.writerow(["Num iters", "Mean rel error"])
csv_results.close()

csv_results = open(f"{dir_checkpoints}/results/results_mean_rel_error_quant_{num_steps}steps.csv", "a", buffering=1)
writer_results = csv.writer(csv_results, delimiter=',')
...
for checkpoint in checkpoints:
  ...
  mean_eror_quant = rollout_quant_vs_dequant_steps.main(model_quant = model_quant, model_dequant = model_dequant, env_name="Pong-v0", steps = num_steps)
  writer_results.writerow([iters_num, mean_eror_quant])
